# 모바일 스토어 배포 현황

기준일: 2026-07-19

## Android 비공개 테스트 후보

| 항목 | 상태 | 위치 또는 기준 |
|---|---|---|
| 패키지 ID | 준비 | com.mypaso.app |
| 버전 | 준비 | 0.2.0 / versionCode 2 |
| 출시명 | 준비 | 0.2.0-alpha2 |
| 최소·대상 SDK | 준비 | API 24 / API 36 |
| App Bundle | 서명·검증 완료 | android/app/build/outputs/bundle/release/app-release.aab |
| 릴리스 APK | 서명·설치 검증 완료 | android/app/build/outputs/apk/release/app-release.apk |
| 업로드 키 | 저장소 밖으로 분리 | C:\Users\sinmb\key |
| 런타임 권한 | 최소화 | 포그라운드 위치, 카메라 |
| 백그라운드 위치 | 미요청 | AndroidManifest 확인 |
| Android 자동 백업 | 차단 | 클라우드 백업·기기 이전 제외 |
| 개인정보처리방침 | 국문·영문 준비 | public/privacy.html |
| 스토어 등록정보 | 국문·영문 준비 | store-assets/store-listing.txt |
| 이미지 | 실제 앱 화면으로 준비 | store-assets 폴더 |
| 테스트 운영 자료 | 준비 | closed-test 폴더 |

## 최종 로컬 검증

- AAB SHA-256: 263E480177C66FC68105536FA9A02542F410C574BC9F7CBF50A6418FE2EF6CBF
- AAB 크기: 7,017,465 bytes
- Bundletool 구조 검증, 업로드 인증서 일치: 통과
- 릴리스 APK SHA-256: F9FC773866FAC286770C1967023F5D351182DA41B6B1B46AEE44C7E0A30C5754
- APK v2 서명 및 업로드 인증서 일치: 통과
- 휴대전화 API 35와 태블릿 API 36에서 서명 APK 설치·콜드 스타트: 통과
- 지도, 탐색, 저장 장소, 태그, 상세 화면, 방문, 사진, 저널, 프로필: 통과
- 앱 강제 종료·업데이트 설치 후 로컬 기록 유지: 통과
- 앱 데이터 초기화 후 JSON 전체 복원: 통과
- 테스트 파일 23개, 테스트 60개, ESLint, 웹·모바일 빌드, Capacitor 동기화: 통과
- npm audit: 취약점 0건

세부 명령, 기기 프로필, 해시와 화면 증거는 closed-test/evidence/report-0.2.0.md에 기록했습니다.

## Play Console 데이터 보안 기준

현재 0.2.0 후보 기준 답변입니다.

- 개발자 또는 제3자 서버로 수집하는 사용자 데이터: 없음
- 제3자와 공유하는 사용자 데이터: 없음
- 광고, 행동 추적, 원격 분석, 계정: 없음
- 위치: 사용자가 현장 확인을 누른 동안에만 기기 안에서 처리
- 사진: 사용자가 방문 사진을 추가할 때만 기기 안에 저장
- 방문, 리뷰, 태그, XP: 앱 전용 로컬 저장소에 보관
- Android 자동 클라우드 백업과 기기 이전: 비활성화
- 사용자가 직접 내보낸 JSON: 좌표, 태그, 방문 기록, 사진을 포함할 수 있으며 별도 암호화되지 않음
- 삭제: 앱 데이터 삭제 또는 앱 제거

Google의 데이터 보안 안내는 기기 안에서만 처리되고 외부 전송되지 않는 데이터 접근을 수집으로 신고하지 않아도 된다고 설명합니다. 단, 외부 동기화·분석·광고·충돌 수집 SDK를 추가하면 새 AAB를 올리기 전에 다시 신고해야 합니다.

## Play Console에서 남은 절차

1. 0.2.0 AAB를 비공개 테스트 Alpha 트랙에 업로드합니다.
2. 권한, 데이터 보안, 앱 액세스, 콘텐츠 등급과 타겟층 답변을 0.2.0 동작에 맞게 갱신합니다.
3. 국문 기본 등록정보와 별도 영문 등록정보, 아이콘, 그래픽, 휴대전화·태블릿 스크린샷을 등록합니다.
4. 출시명 0.2.0-alpha2와 최신 출시 노트를 저장하고 검토에 제출합니다.
5. 개인 개발자 계정에 해당 요건이 표시되면 15명 이상을 모집해 최소 12명이 14일 연속 옵트인하도록 운영합니다.
6. 실제 미션, 피드백, 결함과 수정 결과를 기록한 뒤 프로덕션 액세스를 신청합니다.

## 공개 출시 전 필수 게이트

1. 기능 검증용 더미 POI 100건을 출처·좌표·라이선스가 확인된 실제 데이터로 교체합니다.
2. 새 시드로 스크린샷과 스토어 설명을 다시 검수합니다.
3. 비공개 테스트의 P0·P1 결함을 모두 닫고 회귀 검증을 남깁니다.
4. Play Console이 표시하는 테스터 수와 연속 기간을 충족합니다.
5. 최신 Android System WebView 또는 Chrome이 설치된 대표 실기기에서 한 번 더 확인합니다.

## 호환성과 iOS

Android 최소 버전은 API 24이지만 최신 웹 UI 엔진을 사용하므로 Android System WebView 또는 Chrome 업데이트가 필요합니다. 오래된 AOSP WebView 95에서는 최신 스타일 일부가 잘못 렌더링됐으며, WebView 124·134 환경에서는 정상 동작했습니다.

iOS 프로젝트와 handoff 문서는 준비되어 있습니다. Windows에서는 App Store 아카이브, 코드 서명, iOS Simulator 테스트를 수행할 수 없으므로 macOS와 Xcode에서 컴파일·시뮬레이터·실기기 검증을 이어가야 합니다.
