...
- 제출된 출시: `0.2.0-alpha4`, `versionCode 5`, target SDK 36
- 업로드 파일: `app-release.aab`
- 한국어·영어 출시 노트: 2개 중 2개 언어 입력 및 저장
- 출시 대상: 비공개 테스트 Alpha, 100%
- 검토 제출: 완료, Play 상태 `검토 중인 변경사항`
- 관리형 게시: 사용 중지됨. Google 승인 후 비공개 테스트에 자동 출시
- 빠른 정책·품질 검사: 차단 문제 없이 완료되어 Google 본 검토 단계로 전환
- 지원 기기 감소: 전화·태블릿·TV·차량·Chromebook·Android XR 모두 0대
- Play 예상 신규 설치 크기: 7.05MB
- Play 예상 업데이트 크기: 5.44MB
...
출시 검증에는 오류 없이 경고 2건이 표시됐습니다.

- 가독화 파일 없음: 이 빌드는 `minifyEnabled false`로 R8/Proguard 난독화를 사용하지 않아 매핑 파일이 생성되지 않음
- 네이티브 디버그 기호 없음: AAB에 포함된 네이티브 라이브러리의 기호 파일 업로드 권고

두 항목 모두 비정상 종료·ANR 분석 편의를 위한 비차단 권고입니다. 기존 edge-to-edge 관련 두 문구는 이번 출시 검증 경고 목록에는 표시되지 않았으며, 별도의 Play 재분석 결과를 계속 확인해야 합니다.
