...
> 역사 기록: 이 문서는 현재 게시된 `0.2.0-alpha2` (`versionCode 3`)의 증거입니다. 다음 `0.2.0-alpha3` (`versionCode 4`) 후보의 최신 산출물·실제 POI·화면 검증은 `report-0.2.0-alpha3.md`를 기준으로 합니다. 아래 경로의 로컬 산출물은 후속 빌드로 교체됐으므로 이 문서의 해시와 크기는 당시 제출 스냅샷 값입니다.
...
Android 비공개 테스트용 서명 AAB와 APK를 생성했고, 자동 검증과 휴대전화·태블릿 실제 앱 흐름을 통과했습니다. 로컬 기록의 강제 종료·업데이트 영속성, 민감 로그 차단, JSON 내보내기, 앱 데이터 초기화 후 전체 복원을 확인했습니다. 0.2.0-alpha2 100% Alpha 출시안과 등록정보 변경 3건은 2026-07-19 Google 검토에 제출됐고 같은 날 게시 완료됐습니다.
...
- 제출일: 2026-07-19
- 상태: Google 검토 완료·Alpha 게시 활성
- Alpha 출시안: 0.2.0-alpha2, 100%
- 제출 변경사항: Alpha 출시안, 영어(미국) 등록정보 추가, 한국어 간단한 설명, 한국어 전체 설명
- 지원 기기 감소: 전화 0대, 태블릿 0대, 기타 지원 폼 팩터 0대
- 남은 경고: R8/Proguard를 사용하지 않는 빌드의 가독화 파일 부재 1건
- 관리형 게시: 사용 중지, Google 승인 후 자동 게시 완료
...
2. 후속 versionCode 4에서 더미 POI 100건을 Wikidata CC0 실제 장소 100건으로 교체하고 새 이미지·설명을 검증했습니다. 세부 증거는 `report-0.2.0-alpha3.md`에 있습니다.
3. Play Console의 AAB, 정책 선언, 등록정보와 이미지는 제출·게시됐고 실제 테스터 운영이 진행 중입니다.
